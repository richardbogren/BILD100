...
//100
//150
const SVG_WIDTH_AS_PERCENT_OF_CONTAINER_WIDTH = .35
...
document.querySelector("#matter-container canvas").id = "matter-canvas"
//Turn on scroll on canvas
//mouseConstraint.mouse.element.removeEventListener("mousewheel", mouseConstraint.mouse.mousewheel);
//mouseConstraint.mouse.element.removeEventListener("DOMMouseScroll", mouseConstraint.mouse.mousewheel);

let canvasClearCheck = false

document.addEventListener('keyup', event => {
    if (event.code === 'Space') {
        if (canvasClearCheck == false) {
            svgArr.forEach(function (item, index) {
